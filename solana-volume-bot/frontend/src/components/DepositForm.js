  ...
  function DepositForm({ userPublicKey, setMessage }) {
    ...
    useEffect(() => {
      if (userPublicKey) {
        axios.get(`${process.env.REACT_APP_API_URL}/wallets/get-distribution-wallet?userPublicKey=${userPublicKey.toString()}`)
          .then(response => {
            if (response.data && response.data.distributionWallet) {
              setDistributionWallet(response.data.distributionWallet);
            } else {
    ...
    const handleDeposit = async (e) => {
      ...
      try {
        ...
        const depositResponse = await axios.post(`${process.env.REACT_APP_API_URL}/wallets/deposit`, {
